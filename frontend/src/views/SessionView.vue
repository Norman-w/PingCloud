...
<template>
  <div style="min-height: 100vh; background: #f0f2f5; padding-bottom: 80px;">
      ...
      <template v-if="step === 'play' && currentSession">
        <div style="background: #fff; border-radius: 12px; padding: 16px; margin: 12px 16px; box-shadow: 0 2px 12px rgba(0,0,0,0.06);">
          <div style="display: flex; align-items: center; justify-content: center; gap: 8px;">
            ...
            <template v-else>
              <span style="font-weight: 700; font-size: 18px;">{{ currentSession.name }}</span>
          ...
          <div style="font-size: 13px; color: #969799; margin-top: 6px; text-align: center;">
            {{ currentSession.matches.filter(m => m.played).length }} / {{ currentSession.matches.length }} 场已完成
          </div>
        </div>
        ...
        <div style="padding: 8px 16px;">
          <button @click="showAddPlayerDialog = true; addPlayerId = 0"
            style="width: 100%; padding: 12px; background: #fff; color: #1989fa; border: 2px dashed #1989fa; border-radius: 12px; font-size: 15px; font-weight: 500; cursor: pointer;">
            + 拉人加入本场
          </button>
        </div>
    ...
    <!-- Add Player Dialog -->
    <AddPlayerDialog
      :show="showAddPlayerDialog"
      :players="players"
      :exclude-ids="currentSession?.players.map(p => p.id) || []"
      :session-name="currentSession?.name || ''"
      @update:show="showAddPlayerDialog = $event"
      @add="(pid: number) => { addPlayerId = pid; addPlayerToSession() }"
    />
