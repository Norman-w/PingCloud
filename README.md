...
封闭小圈子长期互打，总分恒定会导致水平提升无法体现在积分上。因此引入**胜者奖励分**机制：

- 每场胜者额外 **+1 分**（不扣败者），向系统注入积分
- 通过环境变量 `RATING_BONUS` 控制（默认 0 = USATT 标准零和，设为 1 启用）
- 持续赢球的人积分自然上涨，反映真实水平进步

### 🚩 弃权规则

- 弃权双方积分不变
- 胜者计胜场，败者**不计负场**（单独统计为「弃权」）
- 胜率计算排除弃权场次
