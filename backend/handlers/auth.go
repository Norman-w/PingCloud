...
// SMS config from env vars
var (
	smsAccessKey    = os.Getenv("ALIBABA_ACCESS_KEY")
	smsAccessSecret = os.Getenv("ALIBABA_ACCESS_SECRET")
	smsSignName     = os.Getenv("ALIBABA_SMS_SIGN")
	smsTemplateCode = os.Getenv("ALIBABA_SMS_TEMPLATE")
)

// In-memory code store: phone → {code, expires, playerId}
var (
	codeStore   = map[string]struct{ code string; expires time.Time; attempts int }{}
...
func smsSignature(params map[string]string, method string) string {
	...
	mac := hmac.New(sha1.New, []byte(smsAccessSecret+"&"))
	mac.Write([]byte(str))
...
func sendSMS(phone, code string) error {
	if smsAccessKey == "" || smsAccessSecret == "" {
	...
	params["Signature"] = smsSignature(params, "GET")

	query := ""
	for k, v := range params { query += "&" + percentEncode(k) + "=" + percentEncode(v) }
...
func AuthSendCode(w http.ResponseWriter, r *http.Request) {
	...
	if len(phone) < 11 {
		http.Error(w, "请输入正确的手机号", http.StatusBadRequest); return
	}
	...
	if pid == 0 {
		http.Error(w, "该手机号未绑定球员", http.StatusNotFound); return
	}
	...
	if s, ok := codeStore[phone]; ok && s.attempts >= 3 && time.Now().Before(s.expires.Add(4*time.Minute)) {
		codeStoreMu.Unlock()
		http.Error(w, "验证码发送次数过多，请稍后再试", http.StatusTooManyRequests); return
	}
	if s, ok := codeStore[phone]; ok && time.Now().Sub(s.expires) > -240*time.Second {
		codeStoreMu.Unlock()
		http.Error(w, "验证码已发送，请60秒后重试", http.StatusTooManyRequests); return
	}
	codeStoreMu.Unlock()

	code := genCode()
	if err := sendSMS(phone, code); err != nil {
		http.Error(w, "短信发送失败: "+err.Error(), http.StatusInternalServerError); return
	}

	codeStoreMu.Lock()
	attempts := 0
	if s, ok := codeStore[phone]; ok { attempts = s.attempts }
	codeStore[phone] = struct{ code string; expires time.Time; attempts int }{code, time.Now(), attempts + 1}
	codeStoreMu.Unlock()
...
func AuthVerify(w http.ResponseWriter, r *http.Request) {
	...
	if !ok || s.code != req.Code || time.Now().After(s.expires.Add(5*time.Minute)) {
		codeStoreMu.Unlock()
		http.Error(w, "验证码错误或已过期", http.StatusUnauthorized); return
	}
...
func AuthMe(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil { http.Error(w, "not logged in", http.StatusUnauthorized); return }

	var pid int
	fmt.Sscanf(cookie.Value, "%d:", &pid)
	if pid == 0 { http.Error(w, "invalid token", http.StatusUnauthorized); return }
